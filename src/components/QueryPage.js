...
const QueryPage = () => {
  ...
  const handleFetchResults = async () => {
    ...
    try {
      // Construct the full legal query
      const fullQuery = `Explain the legal implications of "${query}" under Indian Law, including sections from the IPC and other relevant laws and reference links.`;
      ...
      if (geminiResponse?.data) {
        ...
        if (candidates && candidates.length > 0) {
          ...
          // Split the text into paragraphs and limit to 3
          ...
          if (paragraphs.length > 0) {
            const formattedResponse = paragraphs.join("\n\n");
      ...
      <Container maxWidth="md">
        ...
        {response && (
          ...
          >
            <Typography
              variant="h5"
              sx={{ color: "#2e7d32", mb: "1.5rem", fontWeight: "bold" }}
            >
              Legal Implications (Response from Vidhi Shiksha AI)
            </Typography>
