...
export default function IPCtoBNSForm() {
    ...
    try {
      const [ipcResponse, crpcResponse, ieaResponse] = await Promise.allSettled(
        [
          ...
          axios.post("/api/ieatobsa", { iea: ipc }),
        ]
      );
      ...
      if (ipcResponse.status === "fulfilled") {
        ...
        } else if (!ipcData.bnsMatches?.length) {
          setErrors((prev) => ({
            ...prev,
            ipc: "No matching BNS section found",
          }));
        } else {
      ...
      if (crpcResponse.status === "fulfilled") {
        ...
        if (crpcData.isExactMatch) {
          ...
          setErrors((prev) => ({
            ...prev,
            crpc: "No exact match found for CRPC/BNSS",
          }));
        }
      } else {
      ...
      if (ieaResponse.status === "fulfilled") {
        ...
        if (ieaData.isExactMatch) {
          ...
          setErrors((prev) => ({
            ...prev,
            iea: "No exact match found for IEA/BSA",
          }));
        }
      } else {
      ...
      setErrors({
        ipc: "An unexpected error occurred",
        crpc: "An unexpected error occurred",
        iea: "An unexpected error occurred",
      });
    } finally {
      setLoading(false);
    }
  ...
  const debouncedFetchData = useCallback(
    debounce((ipc) => fetchData(ipc), 300),
    [fetchData]
  );
  ...
  const handleSearch = (term) => {
    ...
    if (term.trim()) {
      ...
      setErrors({
        ipc: "Please enter a valid section number",
        crpc: "",
        iea: "",
      });
    }
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl">
      <h1 className="text-3xl font-bold mb-6 text-center text-white">
        Law Section Converter
      </h1>
      <p className="mb-8 text-center text-gray-400">
        Enter a section number to get corresponding information for IPC/BNS,
      ...
      {errors.ipc && (
        <ErrorDisplay
          error={errors.ipc}
          onDismiss={() => handleDismissError("ipc")}
        />
      )}
      {errors.crpc && (
        <ErrorDisplay
          error={errors.crpc}
          onDismiss={() => handleDismissError("crpc")}
        />
      )}
      {errors.iea && (
        <ErrorDisplay
          error={errors.iea}
          onDismiss={() => handleDismissError("iea")}
        />
      )}
